...
| `DOCUMENT_STORAGE_ROOT` | непубличный каталог локального хранилища документов | `./.runtime/documents` |
| `DOCUMENT_MAX_FILE_SIZE_BYTES` | максимум одного файла, не более 100 МБ | `26214400` |
...
Документы хранятся только на локальной файловой системе. В Docker для них используется
отдельный volume `project1_document_data`; не удаляйте его при обновлении приложения и
включайте в резервное копирование вместе с PostgreSQL. Каталог нельзя размещать внутри
`public`, `.next`, `src` или `node_modules`. S3 и другие платные object-storage сервисы
этому модулю не требуются.
